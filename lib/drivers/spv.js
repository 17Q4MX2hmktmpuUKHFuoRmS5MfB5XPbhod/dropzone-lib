...
var TxoCache = 'TODO: This should be a param'//cache.models.Txo
var TipCache = 'TODO: This should be a param'//cache.models.Tip
...
BloomFilter.prototype.insertAddress = function (address) {
  ...
  this.insert(address.toBuffer().slice(1))
}
...
BloomFilter.forAddress = function (address) {
  var filter = BloomFilter.create(1, 0.0000000000000001)
...
Network.prototype.getFilteredTxs = function (filter, next) {
  ...
  var updateMaxHeight = function (height) {
    heights[height] =
      (heights[height] || 0) + 1
    maxHeight = parseInt(Object.keys(heights)
      .sort(function (a, b) {
  ...
  var betterPeer = function (peer) {
    if (!loader) return true
    var loaderDeprecated =
    loader.bestHeight !== maxHeight
    var peerIsBetter =
    peer.bestHeight === maxHeight &&
      peer.ping < loader.ping
    return loaderDeprecated || peerIsBetter
  }

  var getHeaders = function (peer, blockId) {
    peer.sendMessage(new GetHeaders({
      starts: [blockId],
  ...
  pool.on('peerready', function (peer, addr) {
    peer.hash = addr.hash
    if (peer.bestHeight >= tip.blockHeight) {
      peer.ping = new Date()
      peer.sendMessage(new Ping())
      peer.sendMessage(new FilterLoad(filter))
      updateMaxHeight(peer.bestHeight)
      peer.on('pong', function () {
        peer.ping = new Date() - peer.ping
        if (betterPeer(peer)) {
          loader = peer
          getHeaders(loader, tip.blockId)
        }
      })
    } else {
      peer.disconnect()
  ...
  pool.on('peerdisconnect', function (peer, err) {
    if (loader && loader.hash === peer.hash) {
      loader = null
    }
  })
