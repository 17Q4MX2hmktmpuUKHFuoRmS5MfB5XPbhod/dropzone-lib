...
Network.prototype.getFilteredTxs = function (filter, next) {
  ...
  pool.on('peerready', function (peer, addr) {
    ...
    if (peer.bestHeight >= tip.blockHeight) {
      heights[peer.bestHeight] = 
        (heights[peer.bestHeight] || 0) + 1
      maxHeight = parseInt(Object.keys(heights)
        .sort(function (a, b) { 
          return heights[b] - heights[a] })[0], 10)
      peer.on('pong', function () {
        peer.ping = new Date() - peer.ping
        peer.sendMessage(new FilterLoad(filter))
        var shouldUpdate = true
        if (loader) {
          shouldUpdate = loader.bestHeight !== maxHeight || 
            (peer.bestHeight === maxHeight && peer.ping < loader.ping)
        }
        if (shouldUpdate) {
          loader = peer
          loader.getHeaders = function (blockId) {
            this.sendMessage(new GetHeaders({
              starts: [blockId],
              stops: new Array(33).join('0')
            }))
          }
          loader.getHeaders(tip.blockId)
        }
      })
      peer.ping = new Date()
  ...
  pool.on('peerheaders', function (peer, message) {
    ...
    if (headers.length && header) {
      ...
      return peer.getHeaders(tip.blockId)
    }
