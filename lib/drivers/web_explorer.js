...
WebExplorer.prototype._createTransaction = function (senderAddr, tip, cbOutputs, cb) {
  this.getUtxos(senderAddr, function (err, utxos) {
    ...
    utxos = utxos.sort(function (a, b) { return a.satoshis - b.satoshis })
    ...
    for (var i = 0, l = utxos.length; i < l; i++) {
      ...
      if (utxo.script) {
        ...
        if (txoScpt.isMultisigOut()) {
          ...
          tx.addInput(new MultiSigInputWithoutSort({prevTxId: utxo.txid,
            // TODO: 1 isn't a given, determine from the op, and if not 1, skip
            // this input?
      ...
      } else if (utxo.scriptPubKey) {
        // Some drivers give us pubkeys:
        tx.from({address: senderAddr, txId: utxo.txid,
          outputIndex: utxo.outputIndex, satoshis: utxo.satoshis,
          scriptPubKey: utxo.scriptPubKey})
      } else {
        // Something is up with this utxo, let's see if we can just skip over it:
        continue
      }
...
module.exports = {
  ...
  bitcoinStrToSatoshis: function (bitcoinStr) {
    var bigHundredMil = new BigDecimal('100000000')
    return (new BigDecimal(bitcoinStr)).multiply(bigHundredMil).intValue()
  }
}
