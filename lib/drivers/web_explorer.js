...
function MultiSigInputWithoutSort(input, pubkeys, threshold, signatures) {
  ...
  $.checkState(Script.buildMultisigOut(this.publicKeys, threshold, {noSorting: true}).equals(this.output.script),
  ...
  this.getUtxos(senderAddr, function (err, utxos) {
    ...
    for (var i = 0, l = utxos.length; i < l; i++) {
      ...
      if (txoScpt.isMultisigOut()) {
        ...
        // TODO: Grab this out of : publicKeysWithoutSignature
        pubKeys = txoScpt.chunks.slice(1, 1 + opCount).map(function (pubKey) {
          return PublicKey.fromBuffer(pubKey.buf)
        })

        /*
        tx.addInput(new MultiSigInputWithoutSort({ output: new Output({
...
/*
Vanilla input:
        tx.addInput(new Input({prevTxId: utxo.txid, outputIndex: utxo.outputIndex, 
          script: utxo.script}), utxo.script, utxo.satoshis)
          */
