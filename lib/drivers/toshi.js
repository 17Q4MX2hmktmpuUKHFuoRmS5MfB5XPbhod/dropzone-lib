...
Toshi.prototype.messagesByAddr = function (addr, options, cb) {
  ...
  // TODO: The default limit is 100. Support paging.
  this._req(['api/v0/addresses/', addr, '/transactions?limit=1'], function (err, data) {
    if (err) return cb(err)
    console.log(data)

    this._filterTxs(JSON.parse(data).transactions, loadTx.bind(this), options, cb)
  }.bind(this))
}
