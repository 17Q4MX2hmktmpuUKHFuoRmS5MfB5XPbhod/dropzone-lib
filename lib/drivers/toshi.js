...
Toshi.prototype.getUtxos = function (addr, cb) {
  this._get(['api/v0/addresses/', addr, '/unspent_outputs'], function (err, data) {
    ...
    var utxos = jsonData.map(function(utxo) {
      // TOOD: Nix var script = Script.fromHex(utxo.script_hex).toString()
...
Toshi.prototype.relay = function (rawTx, cb) {
  this._post(['api/v0/transactions'], {hex: rawTx}, function (err, data) {
    ...
    // TODO: https://github.com/coinbase/toshi/blob/master/lib/toshi/processor.rb#L1143
    console.log(data)

    var jsonData = JSON.parse(data)

    // TODO: check for a txid too
    if (jsonData.error) return cb(new RelayUnacceptedError())
