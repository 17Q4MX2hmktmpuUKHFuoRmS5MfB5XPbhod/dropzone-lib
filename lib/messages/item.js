...
var Item = function Item (connection, attrs) {
  ...
  this.__defineGetter__('latitude', function () {
    return (attrs.latitude) ? attrs.latitude
      : this._integerToLatLon(this._addressParts(this.receiverAddr, 0))
  })

  this.__defineGetter__('longitude', function () {
    return (attrs.longitude) ? attrs.longitude
      : this._integerToLatLon(this._addressParts(this.receiverAddr, 1), 180)
  })

  this.__defineGetter__('radius', function () {
    return (attrs.radius) ? attrs.radius
      : this._addressParts(this.receiverAddr, 2)
  })
  ...
  this.__defineGetter__('receiverAddr', function () {
    ...
    } else if (this.latitude && this.longitude && this.radius) {
...
Item.prototype._integerToLatLon = function (latlon, unsignedOffset) {
  ...
  // TODO: Can we nix some of these?
  var bigLatlon = new BigDecimal('latlon')
  var bigMillion = new BigDecimal.BigInteger('1000000')
  var bigOffest = new BigDecimal.BigInteger(String(unsignedOffset))
