...
function NotInitializedError () {
  DropzoneError.call(this, 'the conversation is not yet intialized')
}
  ...
  */
  ...
  if (options.receiverAddr) {
    ...
    */
    ...
    if ((this.withChat.receiverAddr !== this.senderAddr) || (!this.withChat.isInit())) {
 ...
 */
Session.prototype.authenticate = function (cb, derEncoding) {
  this._derAttribs(function (err, attribs) {
    if (err) return cb(err)

    this._chatCreate(attribs, cb)
  }.bind(this), derEncoding)
}
...
Session.prototype.send = function (contents, cb, iv) {
  ...
  this.getNegotiation(function (err, commInit, commAuth) {
    ...
    if (!commInit) return cb(new NotInitializedError())
    if (!commAuth && !this.withChat) return cb(new NotAuthenticatedError())

    var chatAttribs = {}

    async.series([
      // Do we need to include a der?
      function (next) {
        if (!commAuth) {
          this._derAttribs(function (err, attribs) {
            if (err) return cb(err)
        ...
        this.getSymmKey(function (err, symmKey) {
          ...
          chatAttribs.iv = iv
          chatAttribs.contents = Buffer.concat([cipher.update(contents),
            cipher.final()])
          next()
        })
      }.bind(this)], function (err, results) {
        if (err) return cb(err)
        this._chatCreate(chatAttribs, cb)
      }.bind(this))
  }.bind(this))
}
    ...
    function (err, results) {
      ...
      this._getChats({startBlock: commInit.blockHeight},
        function (err, messages) {
          if (err) return cb(err)
          cb(null, messages.filter(function (msg) { return !!msg.contents })
...
Session.prototype._derAttribs = function (next, derEncoding) {
  this.getNegotiation(function (err, commInit, commAuth) {
    ...
    next(null, {sessionPrivKey: dh.getPublicKey(),
      der: (isInit) ? der : null })
  }.bind(this))
}
