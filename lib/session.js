...
Session.prototype.isAuthenticated = function (cb) {
  // TODO: Seriously need to test this creativity
  async.every([this.getCommunicationInit, this.getCommunicationAuth],
    function (m, next) { m.apply(this, next) },
...
Session.prototype.getTheirPrivKey = function (cb) {
  async.detectSeries([this.getCommunicationInit, this.getCommunicationAuth],
    function (m, next) {
      m.apply(this, function (err, msg) {
        if (err) next(err)

        next(null, ((msg) && (msg.senderAddr === this.receiverAddr) &&
          (msg.receiverAddr === this.senderAddr)) ? msg : null)
      })
    }, function (result) { cb(null, result) })
}
...
Session.prototype.symmKey = function (cb) {
  ...
  async.series([
    function (next) { this.getCommunicationInit(next) },
    function (next) { this.getTheirPrivKey(next) }],
    function (err, comms) {
      if (err) cb(err)
      ...
      var der = DHDER.decode(new Buffer(commInit.der), 'der')
...
// deterministic
Session.prototype.authenticate = function (next, der) {
  ...
  async.series([
    ...
    function (err, comms) {
      ...
      // somehow compromised
      if (!isInit && !!der) throw new DerAlreadyExistsError()

      if (!der && this.withChat) { 
        der = this.withChat.der 
// TODO: Grabbing the der from withChat is where we're off
        console.log("todo:")
        console.log(util.inspect(der))
      }
      ...
      if (isInit) {
        ...
        der = DHDER.decode(new Buffer(commInit.der), 'der')
        p = der.p.toString(16)
        g = parseInt(der.g.toString(10), 10)
        dh = crypto.createDiffieHellman(p, 'hex', g)
      }
      ...
      der = DHDER.encode({ p: new BN(dh.getPrime('hex'), 16),
