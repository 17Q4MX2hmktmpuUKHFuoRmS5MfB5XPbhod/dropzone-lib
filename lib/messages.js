...
Message.isUnique = function (a, x, messages) {
  return !messages.filter(function (b, y) {
    return Message.isValid(b) && a.txId === b.txId && x > y
  }).length
}
...
ChatMessage.prototype.decrypt = function (symmKey) {
  var key = symmKey.slice(0, 32)
...
messages.watch = function (addr, next) {
  blockchain.watchTxsByAddr(addr, function (err, tx) {
    if (err) return next(err)
    var message = Message.fromTx(tx, addr.network)
...
messages.InsufficientBalanceError = InsufficientBalanceError
